...
def test_candidate_generation_manifest_binds_external_skillopt_output(tmp_path: Path):
    ...
    assert manifest["status"] == "fixture_only_not_exportable"
    assert manifest["requires_approved_export"] is False
    ...
    assert manifest["authoritative_use"] is False


def test_candidate_generation_manifest_rejects_best_skill_and_materialization_drift(
    tmp_path: Path,
):
    materialization_path = _materialization_manifest_path(tmp_path)
    best_skill = _best_skill(tmp_path)
    ...
    best_skill.write_text(
        best_skill.read_text(encoding="utf-8") + "\ndrift\n", encoding="utf-8"
    )
...
def test_candidate_generation_manifest_rejects_external_run_extra_fields_and_hash_drift(
    tmp_path: Path,
):
    materialization_path = _materialization_manifest_path(tmp_path)
    best_skill = _best_skill(tmp_path)
    ...
    with pytest.raises(ValidationError, match="unsupported keys"):
        record_candidate_generation_manifest(
            ...
            external_run={
                **_external_run(),
                "raw_logs": "USER_EMAIL=alice@example.com",
            },
        )
...
def test_candidate_generation_rejects_secret_or_raw_log_external_run(tmp_path: Path):
    with pytest.raises(ValidationError, match="secret|raw-user-log|raw user logs"):
        record_candidate_generation_manifest(
            ...
            external_run={
                **_external_run(),
                "command": "python train.py --api_key secret",
            },
        )
    with pytest.raises(ValidationError, match="raw user logs"):
        record_candidate_generation_manifest(
            output_dir=tmp_path / "candidate-generation-raw",
            materialization_manifest_path=_materialization_manifest_path(
                tmp_path / "raw"
            ),
...
def test_legacy_candidate_generation_cannot_be_relabelled_or_auto_upgraded(
    tmp_path: Path,
) -> None:
    manifest = record_candidate_generation_manifest(
        output_dir=tmp_path / "candidate-generation",
    ...
    relabelled = {**manifest, "version": "skillopt-candidate-generation-manifest-v2"}
    with pytest.raises(ValidationError, match="version"):
        validate_candidate_generation_manifest(relabelled)

    parameters = inspect.signature(export_approved_skillopt_policy).parameters
    assert "external_run" not in parameters
    assert "best_skill_path" not in parameters
    assert "materialization_manifest_path" not in parameters
