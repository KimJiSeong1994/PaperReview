...
본 논문은 6개 학술 데이터베이스를 병렬로 검색하고, LLM 기반 갭 분석을 통해 부족한 영역을 자율적으로 보완하는 멀티턴 검색 에이전트 시스템을 제안한다. 기존 단일 쿼리 검색의 커버리지 한계를 극복하기 위해 QueryAnalyzer, HybridRanker(BM25+Semantic+Citations+Recency), 4단계 중복 제거, ReAct 멀티턴 루프, 그리고 RaR-Implicit 기반 Rubric 평가를 통합한 파이프라인을 설계하였다. 실험적으로 단일 턴 대비 멀티턴 검색이 커버리지를 유의미하게 향상시키며, Rubric 기반 자가 평가가 검색 결과의 체계적 품질 보증을 가능하게 함을 보인다.
...
초기 시스템은 6개 소스 병렬 검색과 하이브리드 랭킹으로 커버리지를 확보했으나, 프로덕션 운영 과정에서 다음 세 가지 한계가 드러났다.
...
본 연구는 [집현전(jiphyeonjeon.kr)](https://jiphyeonjeon.kr) 서비스의 검색 인프라를 점진적으로 발전시키며 위 문제들을 해결한 과정을 기술한다. 주요 기여는 다음과 같다.

1. QueryAnalyzer + HybridRanker + 4단계 중복 제거를 결합한 다중 소스 검색 파이프라인
2. ReAct 패러다임 기반 멀티턴 자율 검색 에이전트
3. RaR-Implicit Rubric을 활용한 검색 결과 자가 평가 체계
4. 단계별 타임아웃 분리를 통한 graceful degradation 아키텍처
...
**ArxivQA** [3]는 검색 에이전트 학습에서 reward 설계의 중요성을 보였다. Outcome reward("최종 답이 맞으면 +1, 틀리면 0")는 멀티턴 검색에서 불안정했다. 올바른 과정을 거쳤어도 마지막 답이 살짝 다르면 보상을 받지 못하고, 엉뚱한 방법으로 우연히 맞아도 보상을 받았다. 이에 대한 대안으로 **RaR(Rubric-as-Reward)** 방식이 제안되었다. 결과가 아니라 과정을 평가하는 루브릭 기반 reward를 도입하자, 에이전트가 체계적인 검색 전략을 학습하기 시작했다. 본 연구는 이 RaR 루브릭을 RL fine-tuning 없이 inference-time 평가에 적용한다.

**Search-R1** [2]은 `search -> result -> think -> search` 루프를 통해 LLM이 검색 엔진과 상호작용하며 추론하는 프레임워크를 제안하였다. 본 연구의 ReAct 멀티턴 루프는 이 패러다임을 프롬프트 엔지니어링으로 재현한다.

### 3.2 하이브리드 검색 및 랭킹

**HyDE(Hypothetical Document Embeddings)** [4]는 LLM이 생성한 가상 문서를 쿼리 벡터로 활용하여 zero-shot dense retrieval 성능을 향상시켰다. 본 시스템의 HybridRanker에서 HyDE 모드를 선택적으로 활성화할 수 있다.

**RRF(Reciprocal Rank Fusion)** [5]는 여러 독립 랭킹의 순위 역수를 합산하여 특정 신호에 편향되지 않는 통합 랭킹을 생성한다. 본 시스템은 BM25, Semantic, Citations, Recency 4개 신호를 RRF로 통합한다.

### 3.3 ReAct 프레임워크

**ReAct** [1]은 추론(Reasoning)과 행동(Acting)을 교차하여 LLM의 과제 수행 능력을 향상시키는 프레임워크이다. 본 연구는 이 패러다임을 검색 도메인에 특화하여, 검색 도구 호출과 갭 분석을 교차 실행하는 멀티턴 에이전트를 구현하였다.

---

## 4. Methodology

### 4.1 QueryAnalyzer: 쿼리 분석 및 의도 분류
...
### 4.2 HybridRanker: 다중 신호 기반 하이브리드 랭킹
...
- **Semantic (dense)**: 코사인 유사도. **HyDE** [4]가 활성화되면 LLM이 가상 초록을 생성해 쿼리 벡터를 보강한다.
- **Citations**: log 정규화로 고인용 논문의 지배를 억제한다.
- **Recency**: 연도 기반 계단식 점수 (1년 이내 1.0 ~ 10년 초과 0.1).

이 신호를 통합하는 기본 방식은 **RRF(Reciprocal Rank Fusion)** [5]다. 각 신호로 독립 정렬한 뒤 순위 역수를 합산하므로, 특정 신호에 점수가 몰리지 않고 여러 관점에서 고르게 높은 논문이 우선된다.
...
중복으로 판정되면 메타데이터가 풍부한 쪽을 대표로 삼고 나머지 필드를 병합한다.
...
Search-R1 [2]의 `search -> result -> think -> search` 루프를 RL fine-tuning 없이 프롬프트 엔지니어링으로 재현한 `ReActSearchAgent`를 구현하였다. 전체 타임아웃은 `_TOTAL_TIMEOUT_SECONDS = 120`초이며, 최대 3턴이 기본값이다.
...
*Table 2. 난이도 분류 기준 및 대응 전략. 현재 `/api/deep-search`는 난이도와 무관하게 항상 3턴을 실행하며, 난이도별 전략 분기는 향후 과제이다.*

### 4.6 RaR-Implicit Rubric 기반 결과 평가

검색이 끝나면 `RubricEvaluator`가 결과 세트 전체를 평가한다. 개별 논문이 아니라 **집합으로서의 품질**을 본다. ArxivQA [3]의 RaR-Implicit에서 영감을 받은 이 평가 체계는 4개 차원으로 구성된다.
...
```
Algorithm 1: RaR-Implicit 최종 점수 계산
──────────────────────────────────────
...
본 시스템의 정량적 벤치마크는 향후 과제로 남겨두었다. 현재까지의 운영 경험에서 관찰된 정성적 결과는 다음과 같다.

- 멀티턴 검색은 단일 턴 대비 평균적으로 더 넓은 하위 주제를 커버한다 (Turn 2에서 Turn 1과 겹치지 않는 논문이 추가됨).
- Rubric 평가의 `weakest_dimension` 피드백을 기반으로 한 보완 검색은 해당 차원의 점수를 개선하는 경향을 보인다.
- 타임아웃 분리 후 사용자 대면 에러율이 감소하였다.

---

## 7. Discussion

본 시스템의 설계는 몇 가지 trade-off를 내포한다.

**LLM 의존성.** 갭 분석과 Rubric 평가 모두 LLM에 의존하므로, LLM의 응답 품질과 지연 시간이 전체 시스템 성능에 직접적 영향을 미친다. Fallback 경로를 통해 LLM 장애 시에도 기본적인 검색 기능은 유지되지만, 멀티턴의 핵심 가치인 "반성적 재검색"은 상실된다.

**Recall 측정의 근본적 어려움.** 정답 세트가 없는 open-domain 검색에서 recall을 정량화하는 것은 본질적으로 어렵다. Rubric 평가는 이에 대한 근사치를 제공하지만, 체계적 벤치마크 구축이 필요하다.

**난이도 기반 전략 분기의 미구현.** 현재 모든 deep-search 쿼리에 동일하게 3턴을 실행하므로, 단순 쿼리에 대한 불필요한 지연이 발생한다.

---

## 8. Conclusion

본 논문은 다중 학술 데이터베이스를 대상으로 한 멀티턴 검색 에이전트 시스템을 제안하였다. QueryAnalyzer를 통한 의도 분석, HybridRanker의 다중 신호 랭킹, ReAct 패러다임 기반 자율 재검색, 그리고 RaR-Implicit Rubric 평가를 통합하여 단일 쿼리 검색의 커버리지 한계를 극복하였다. 단계별 타임아웃 분리와 graceful degradation을 통해 프로덕션 환경에서의 안정성을 확보하였다.

향후 연구 방향으로는 (1) 난이도 기반 전략 분기의 실제 적용, (2) 체계적 recall 벤치마크 구축, (3) 사용자 피드백 기반 Rubric 가중치 자동 조정을 계획하고 있다.
...
[2] Jin, Z. et al. (2025). *Search-R1: Training LLMs to Reason and Leverage Search Engines with Reinforcement Learning*. arXiv:2503.09516.

[3] Peng, B. et al. (2023). *ArxivQA: Long-form Question Answering on arXiv Papers*. arXiv:2309.01536.
