...
- `GET /api/recommendations/notifications?limit=5`: ordered visible items, independent unread/total counts, run/time/mode, state/freshness, source statuses and degradation reasons. Limits are 1–5. Unsafe artifacts or unavailable policy return HTTP 503, never old raw data. The UI clears cached cards on refresh failure.
