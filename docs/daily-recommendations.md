...
- `GET /api/recommendations/notifications?limit=5`: ordered visible items including the validated abstract, independent unread/total counts, run/time/mode, state/freshness, source statuses and degradation reasons. Divider-separated paper rows show a whitespace-normalized abstract excerpt (up to 280 characters, two visual lines); missing abstracts are labeled `초록 없음`, never invented or generated through a model call. The title is the sole viewer control and automatically records an acknowledged `seen` as `열어봄`, not proof of completed reading. Related search and interest remain direct one-click commands. The purpose-labeled `추천 조정` chooser contains only hide/already-seen/topic-less; it is not a generic action accordion. Interest is not presented as a persisted toggle because GET does not expose that state. Publication date or year is shown once; technical diagnostics and per-paper provenance follow the list. Explicit feedback has a compact undo receipt; automatic viewer-open receipts stay in the accessible session history. Missing delivery is distinguished from an empty result. Failed feedback remains visible across unrelated refreshes, and `변경 다시 저장` resends the same run/key/action/request ID rather than merely refreshing. Limits are 1–5. Unsafe artifacts or unavailable policy return HTTP 503, never old raw data. The UI clears cached cards on refresh failure.
