...
- `GET /api/recommendations/notifications?limit=5`: ordered visible items including the validated abstract, independent unread/total counts, run/time/mode, state/freshness, source statuses and degradation reasons. Cards show a whitespace-normalized abstract excerpt (up to 280 characters, four visual lines) instead of the generic personalization reason; missing abstracts are disclosed, never invented or generated through a model call. Reading/search actions remain prominent; feedback and technical diagnostics use keyboard-operable disclosure controls. Missing delivery is labeled as not yet generated, distinct from an empty result. Limits are 1–5. Unsafe artifacts or unavailable policy return HTTP 503, never old raw data. The UI clears cached cards on refresh failure.
