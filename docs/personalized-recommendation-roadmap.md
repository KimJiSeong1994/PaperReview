# 개인화 논문 추천 고도화 초기 검토 기록

> **구현 전 설계 기록입니다. 현재 동작이나 실행 지침이 아닙니다.**
> 현행 계약과 명령은 [일일 추천 운영 문서](daily-recommendations.md),
> 평가·출시 조건은 [현행 추천 로드맵](personalized-paper-recommendation-roadmap.md)을 따릅니다.
> 아래의 `raw.json`, username 소유권, OpenClaw 우선 게시, `--skip-existing`,
> 비공개 텍스트 기반 공개 수집, 무시된 노출의 부정 라벨 제안은 폐기되었습니다.
> 현재는 incarnation 소유권, 후보 전용 외부 입력, 단일 공통 게시자,
> 영속 정책을 적용한 Top5와 공개 고정 시드만 사용합니다.

## 결론 요약

이 초기 검토 당시 추천 표면은 `raw.json` 알림 계약을 중심으로 구성되어 있었고, 로컬 추천기는 북마크 텍스트와 후보 논문 텍스트의 토큰 중복에 크게 의존했다. 검토한 축은 **(1) 사용자 행동 이벤트를 추천 피처로 연결**, **(2) 후보 생성과 랭킹을 분리**, **(3) 하이브리드/임베딩/그래프 신호의 후속 검토**, **(4) 오프라인-온라인 평가와 안전한 배포 루프**였다.

이하 내용은 당시의 제안과 한계 분석을 보존한 것이며, 현재 구현·검증·출시 상태를 주장하지 않는다. 새 임베딩/LLM 호출과 자동 승격은 현행 변경에 포함되지 않는다.
