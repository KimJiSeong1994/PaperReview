...
[![같은 Oasis 공연 사실을 원문 문맥에서 복원하는 encoding 검사와 정방향·역방향으로 묻는 knowledge 검사](/api/blog/figures/wikiprofile-fig2-encoding-and-knowledge.png)](/api/blog/figures/wikiprofile-fig2-encoding-and-knowledge.png)
...
[![Wikipedia 사실 추출에서 정·역방향 질문 생성, 검색 기반 필터링, 재표현과 객관식 생성으로 이어지는 파이프라인](/api/blog/figures/wikiprofile-fig3-creation-pipeline.png)](/api/blog/figures/wikiprofile-fig3-creation-pipeline.png)
...
[![13개 LLM에서 encoding 실패, recall 실패, 직접 회상, thinking 회상 등의 비중을 나눈 누적 막대그래프](/api/blog/figures/wikiprofile-fig4-knowledge-profiles.png)](/api/blog/figures/wikiprofile-fig4-knowledge-profiles.png)

*원논문 Figure 4. 작은 Gemma3 모델에서 크게 보이는 노란 encoding 실패 영역이 강한 모델에서는 줄지만, 빨간 recall 실패 영역은 남는다. 진한 초록색은 직접 회상, 연한 초록색은 thinking으로 회복한 profile이다. 검은 선과 상단 Knows는 세 성공 profile의 합이며 encoding 비율이 아니다. 아래 표의 정확한 수치는 Table 2 Selected를 기준으로 한다. Calderon et al. (2026), [arXiv v2, PDF p. 6](https://arxiv.org/pdf/2602.14080v2#page=6). 도판 영역을 직접 추출했으며 축·범례·표시값은 유지했다(CC BY-SA 4.0).*
...
[![Gemini-3-Flash, GPT-5, GPT-4.1의 고인기·저인기 사실에서 encoding과 조건부 회상률 비교](/api/blog/figures/wikiprofile-fig5-fact-popularity.png)](/api/blog/figures/wikiprofile-fig5-fact-popularity.png)
...
[![정방향과 역방향 질문의 객관식 verification 및 자유응답 generation 결과 비교](/api/blog/figures/wikiprofile-fig6-reversal-curse.png)](/api/blog/figures/wikiprofile-fig6-reversal-curse.png)
...
[![Thinking 없이 실패한 사실을 encoding 성공 여부로 나누어 본 thinking 회복률](/api/blog/figures/wikiprofile-fig8-thinking-recovery.png)](/api/blog/figures/wikiprofile-fig8-thinking-recovery.png)
...
[![인기도와 질문 방향에 따른 기본 회상률 위에 thinking의 추가 이득을 쌓은 막대그래프](/api/blog/figures/wikiprofile-fig7-thinking-gains.png)](/api/blog/figures/wikiprofile-fig7-thinking-gains.png)
...
[![정답 비율 문턱 p에 따라 thinking과 base 모델의 correct≥p를 비교한 12개 패널](/api/blog/figures/wikiprofile-fig12-availability-robustness.png)](/api/blog/figures/wikiprofile-fig12-availability-robustness.png)
