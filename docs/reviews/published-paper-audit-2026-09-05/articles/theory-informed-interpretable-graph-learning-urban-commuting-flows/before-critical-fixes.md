...
# PIG-GNN 심층 분석: 통근 흐름 예측에 중력 법칙을 어떻게 넣을 것인가

**Paper:** Zhao, M., Zhang, D., Shi, Z., & Wu, C. (2026). *Theory-informed and interpretable graph learning for urban commuting flows*. Sustainable Cities and Society, 148, 107575. [Publisher](https://doi.org/10.1016/j.scs.2026.107575)

**Abstract:** 도시의 통근 흐름은 유연한 예측 모델만으로 설명하기 어렵다. 출발지는 통근자를 내보내는 생산 역할을, 도착지는 일자리를 끌어들이는 흡인 역할을 하며, 두 지역 사이의 흐름은 거리가 멀어질수록 감소하는 경향을 보인다. PIG-GNN은 이 비대칭을 두 개의 graph encoder로 분리하고, 거리 감쇠·Zipf scaling·흐름 분포를 soft constraint로 학습 목적에 넣는다. England MSOA 수준의 Home-to-Work 자료에서 저자 보고 기준 RMSE 0.297, CPC 0.791, $R^2$ 0.394를 기록했다. 중요한 점은 이 논문이 중력 모형을 신경망으로 대체했다는 데 있지 않다. **지리 이론을 입력 feature가 아니라 architecture와 loss의 제약으로 사용하면서도, 고정된 함수형을 강요하지 않는 절충안**을 제시했다는 데 있다. 다만 여기서 “physics-informed”는 보존 법칙이나 미분방정식을 만족시킨다는 뜻이 아니라, 여러 이동 자료에서 관찰된 통계적 scaling 범위를 정규화 항으로 사용한다는 뜻이다.
...
*원논문 Figure 1. 입력 자료에서 spatial adjacency graph와 directed OD graph를 만들고, theory-informed structure와 physics-informed loss를 거쳐 예측·평가·regime 분석으로 이어지는 전체 절차다. 학술적 비평을 위해 원본을 직접 인용했다. 출처: Zhao et al. (2026), Figure 1.*
...
*원논문 Figure 5. 왼쪽은 지리적으로 인접한 지역을 연결한 spatial adjacency graph, 오른쪽은 실제 이동 방향을 가진 OD flow graph다. 두 graph는 같은 지역 집합을 공유하지만 edge의 의미가 다르다. 학술적 비평을 위해 원본을 직접 인용했다. 출처: Zhao et al. (2026), Figure 5.*
...
$f_P$와 $f_A$는 각각 GCN과 GAT 계층을 포함하지만 parameter를 공유하지 않는다. $z_i^P$는 지역 $i$의 production embedding, $z_i^A$는 attraction embedding이다. 같은 지역 $i$도 두 역할에 따라 서로 다른 vector를 갖는다.
...
*원논문 Figure 6. Production·attraction dual tower, distance-aware GATv2 기반 OD interaction GNN, directed edge MLP, physics-informed loss가 연결된 상세 구조다. 학술적 비평을 위해 원본을 직접 인용했다. 출처: Zhao et al. (2026), Figure 6.*
...
*원논문 Figure 8. Data fidelity, distance-decay slope, Zipf prior, KL divergence를 결합하고 empirical interval 밖의 값에 quadratic penalty를 적용한다. 각 $\lambda_k$는 loss ratio에 따라 조정된다. 학술적 비평을 위해 원본을 직접 인용했다. 출처: Zhao et al. (2026), Figure 8.*
...
Flow 크기의 complementary cumulative distribution이 power law와 유사한 꼴을 보인다는 prior다.

$$
P(Y>x)\propto x^{-\alpha},
\qquad
\alpha\in[0.5,3.0]
$$
...
\mathcal{L}_{KL}
=
D_{KL}\!\left(P(\hat Y)\,\|\,Q(Y)\right)
$$

예측 flow 분포 $P(\hat Y)$가 거리·Zipf prior로 정의된 target distribution $Q(Y)$와 크게 어긋나지 않도록 한다. Slope와 Zipf가 exponent 수준의 조건이라면 KL 항은 분포 전체의 형태를 맞춘다.

$\lambda_k$는 고정 상수가 아니라 loss ratio에 따라 동적으로 조정된다. 이 장치는 data loss보다 scale이 큰 prior 하나가 optimization을 지배하거나, 반대로 prior gradient가 사실상 사라지는 문제를 줄이려는 것이다.
...
*원논문 Figure 7. Intra-city, urban-regional, macro scale 자료에서 distance-decay exponent, KL divergence, Zipf exponent, monotonicity strength를 비교한다. 점선 구간 $\beta\in[0.2,3.0]$, $\alpha\in[0.5,3.0]$이 PIG-GNN의 soft prior가 된다. 학술적 비평을 위해 원본을 직접 인용했다. 출처: Zhao et al. (2026), Figure 7.*
...
주 실험은 England의 MSOA 수준 Home-to-Work commuting flow를 대상으로 한다. 비교군에는 unconstrained·doubly constrained·hurdle·PPML gravity, radiation model, Random Forest, XGBoost, MLP, mixture density network, 여러 graph·deep mobility model이 포함된다.

| Model | RMSE ↓ | MAE ↓ | $R^2$ ↑ | CPC ↑ |
| --- | ---: | ---: | ---: | ---: |
| PIG-GNN | **$0.297\pm0.002$** | **$0.187\pm0.009$** | **$0.394\pm0.014$** | **$0.791\pm0.004$** |
| Unconstrained gravity | — | — | $0.284\pm0.002$ | — |
| GravityGNN | — | — | $0.351\pm0.056$ | — |

*원논문 Table 3과 본문에 공개된 저자 보고 값이다. 대시 표시는 이 리뷰에서 원문 수치를 확인하지 못해 옮기지 않은 항목이다.*

![PIG-GNN 대비 전체 baseline의 상대 성능](/api/blog/figures/pig-gnn-fig9-performance.jpg)

*원논문 Figure 9. PIG-GNN을 1.0으로 정규화한 상대 성능이다. RMSE·MAE는 작을수록 좋고 $R^2$·CPC는 클수록 좋으므로, 방향을 통일한 상대 비교로 읽어야 한다. 학술적 비평을 위해 원본을 직접 인용했다. 출처: Zhao et al. (2026), Figure 9.*
...
CPC가 1에 가까울수록 전체 통근량의 분배가 관측치와 비슷하다. PIG-GNN이 error metric과 CPC에서 함께 강하다는 것은 단순히 일부 큰 flow만 맞춘 것이 아니라 총량 overlap도 개선했음을 뜻한다.
...
Figure 9는 폭넓은 baseline을 포함한다는 장점이 있다. 반면 주 결과는 한 국가의 한 종류 OD system에 집중된다. 코드·학습 split·hyperparameter를 독립적으로 재실행한 결과가 아니므로 본문의 수치는 모두 저자 보고로 한정해야 한다. 특히 작은 standard deviation은 반복 실행의 안정성을 보여 줄 수 있지만, 다른 도시·시간·zoning system으로의 외적 타당성을 보장하지 않는다.
...
*원논문 Figure 12. Test set의 predicted log flow를 두 cluster로 분리한다. Figure 표기 기준 intra-city cluster는 11,697개, inter-city cluster는 703,418개이며, threshold 5.0을 사용한다. 고유량 cluster는 거리에 둔감한 포화 구간, 저유량 cluster는 거리 증가에 따라 감소하는 구간으로 나타난다. 학술적 비평을 위해 원본을 직접 인용했다. 출처: Zhao et al. (2026), Figure 12.*
...
- 주 검증은 England MSOA-level Home-to-Work flow에 집중된다.
- $\beta$와 $\alpha$ interval은 여러 OD system에서 보정했지만, model transfer 자체를 여러 국가에서 검증한 결과와는 다르다.
...
이 설계가 제공하는 핵심 이점은 제약과 유연성의 분리다. Model은 고정된 중력 함수에 갇히지 않지만, 학습 결과가 알려진 거시적 이동 regularity에서 크게 벗어나면 penalty를 받는다. 저자 보고 결과에서 이 조합은 고전적 gravity, tree model, deep·graph baseline보다 전반적으로 좋은 error와 flow overlap을 보였다.
