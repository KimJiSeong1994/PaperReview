...
| 핵심 한계  | CQ에서 계층 요약이 국소 사실을 가려 baseline에 지는 열이 있고(NQ+DeepSeek-R1에서 Local Search CQ 23.20% > Deep GraphRAG 19.60%), 훈련되지 않은 1.5B는 21.64%로 붕괴해 향상은 72B 교사 증류 SFT와 DW-GRPO를 합친 2단계 학습에서 나온다. 그래프·데이터 규모(엔티티·커뮤니티 수)는 미보고. |
...
세 단계를 관통해 후보를 고르는 것은 beam width $k=3$의 coarse-to-fine beam search다(그림 1의 도해는 $k=2$ 예시로 그려져 있으나 본문과 실험 설정은 $k=3$이다). 탐색은 계층 $C$를 위에서 아래로 훑으며, 각 수준에서 상위 $k=3$개의 후보만 남겨 다음 수준으로 내려간다. 이 고정 폭의 빔은 각 계층에 남길 후보 수를 제한한다. 전체 비용은 계층 깊이와 확장 후보·재랭킹에 따라 달라진다. 폭을 좁게 두어 효율을, 매 수준에서 여러 갈래를 유지해 국소 최적 회피를 노린다.
...
세 번째 축은 효율이다. 논문은 NQ에서 Deep GraphRAG가 DRIFT Search 대비 지연시간을 local 질문에서 86%, global 질문에서 81.6% 줄였다고 보고한다. beam width를 3으로 고정한 계층 탐색이 재귀적 DRIFT보다 계층 탐색과 재랭킹을 사용하는 시스템의 지연 감소로 보고되며, 방문 노드 수 감소의 독립적인 기여를 분리한 절제는 아니다.
