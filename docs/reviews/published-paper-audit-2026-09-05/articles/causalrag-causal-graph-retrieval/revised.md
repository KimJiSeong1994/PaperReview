...
**Paper:** Nengbo Wang; Xiaotian Han; Jagdip Singh; Jing Ma; Vipin Chaudhary (2025). "CausalRAG: Integrating Causal Graphs into Retrieval-Augmented Generation". https://aclanthology.org/2025.findings-acl.1165/ · https://doi.org/10.18653/v1/2025.findings-acl.1165 · arXiv:2503.19878
...
이 방법은 정식 인과 식별보다 **LLM 매개 인과 경로 검색(LLM-mediated causal-path retrieval)**에 가깝다. “인과 그래프를 사용한다”는 표현은 파이프라인의 목적과 프롬프트를 설명할 뿐, 그래프의 간선이 현실의 인과관계를 보증한다는 뜻은 아니다.
...
특히 히트맵의 전체 최댓값은 **$k=5,s=3$의 0.842**다. 본문은 “가장 높은 값인 $k=5,s=5$가 최선의 결과”라고 서술하지만, Figure 6에서 $k=5,s=5$의 값은 0.824다. 본문과 Figure 6의 표시값이 일치하지 않는다.
...
질문은 LLM으로 생성하고 답이 본문에 존재하도록 제한하지만, 질문 생성 모델의 구체적 이름은 보고하지 않는다. Retrieval-time 및 자동평가 호출에는 GPT-4o-mini가 사용된다. 이 자동평가는 사람 평가와 구별해야 하며, 생성과 평가에 같은 모델을 사용하는 조건의 영향도 고려해야 한다.
