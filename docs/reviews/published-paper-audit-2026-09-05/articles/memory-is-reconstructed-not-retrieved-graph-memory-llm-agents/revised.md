...
**Paper:** Shuo Ji; Yibo Li; Bryan Hooi (2026). "Memory is Reconstructed, Not Retrieved: Graph Memory for LLM Agents". https://arxiv.org/abs/2606.06036 · arXiv:2606.06036v1

**Abstract:** MRAgent는 LLM 에이전트의 기억 접근을 Cue–Tag–Content 그래프 위의 다단계 재구성 과정으로 모델링한다. 검색 중 얻은 증거에 따라 다음 연산과 검색 방향을 바꾸며 필요한 기억을 모아 답변한다. LoCoMo와 LongMemEval의 성능표에서는 다섯 수동 기준선보다 높은 값을 보고한다. 비용표는 별도의 네 memory baseline을 비교하며, MRAgent 118k tokens는 그중 최소값 245k보다 낮다. 능동 검색 정책의 기여와 그래프 구조의 기여를 분리하는 절제, 표현력 정리의 조건, 보고된 F1 정의의 일관성은 결과 해석의 한계다.
