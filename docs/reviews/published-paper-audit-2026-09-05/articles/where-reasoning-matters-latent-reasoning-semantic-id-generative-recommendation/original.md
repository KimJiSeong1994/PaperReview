...
*그림 1. Beauty·Instruments·MicroLens의 시맨틱 ID 위치별 평균 정보이득. 출처: Yang et al. (2026), Figure 2, [arXiv:2607.12425v1](https://arxiv.org/abs/2607.12425v1). 원문 도판을 비평·해설을 위해 캡처·축소.*
...
*그림 2. IBA의 2단계 학습, refinement gain predictor, 위치별 동적 스텝 배분. 출처: Yang et al. (2026), Figure 3, [arXiv:2607.12425v1](https://arxiv.org/abs/2607.12425v1). 원문 도판을 비평·해설을 위해 캡처·축소.*
...
여기에는 확인되지 않은 동적 문제가 하나 남는다. gain target은 Stage 1에서 만들지만 Stage 2는 선택된 allocation으로 recommender를 계속 바꾼다. 그러면 Stage 1에서 측정한 loss 감소가 Stage 2 후반의 모델에도 같은 의미를 갖는지, 즉 target이 stale해지지 않는지 점검할 필요가 있다. 논문은 Stage 2 이후 predictor calibration이나 실제 사용자별 할당 분포를 보고하지 않는다. 이는 실험 설정에서 직접 따라오는 추론이지 논문이 스스로 검증한 결론은 아니다.
...
이 표만으로 exact reproduction이 완성되지는 않는다. Eq. (16)–(21)의 $K_{max}$와 전체 Stage 2 loss의 $\lambda_{gain}$ 수치가 v1 구현 세부에 없다. RQ-VAE의 codebook 크기와 tokenizer 학습 세부도 본문의 설명만으로는 충분하지 않으며, 2026년 9월 5일 기준 arXiv 페이지에 공식 코드나 프로젝트 링크가 없다.
...
Stage 1에서 만든 목표값이 Stage 2의 변화한 recommender에서도 최신 상태를 유지하는지 역시 열린 검증 항목이다. 이는 2단계 설정에서 도출되는 가능성이며, 논문이 실제 시의성 문제를 관측했다고 주장하는 것은 아니다.
...
2026년 9월 5일 기준 arXiv 페이지에는 공식 코드나 프로젝트 링크가 없고 일부 구현값도 빠져 있다. 논문이 말하는 reasoning도 명시적 사고 과정이 아니라 hidden-state refinement이므로, 인간형 추론 능력의 향상으로 확대 해석해서는 안 된다.

### 8.4 실제 적용 전에 확인할 조건

| 적용 조건 | 이유 |
| --- | --- |
| semantic ID 위치가 계층적 정보 구조를 가질 것 | IBA의 전역 사전값은 위치별 정보이득 차이를 전제로 한다. |
| 위치별 refinement를 독립적으로 제어할 수 있을 것 | allocator가 개입하는 실행 단위가 token position이다. |
| 자체 데이터에서 IG와 스텝 효용을 다시 측정할 것 | MicroLens처럼 앞의 두 위치 순서가 뒤집힐 수 있다. |
| $\gamma_{rec}$·$\lambda_{LA}$·$\lambda_P$를 별도 validation으로 선택할 것 | 보고된 결과가 일부 계수에 민감하다. |
| 실제 할당 분포와 predictor calibration을 기록할 것 | 사전값과 사용자 적응의 기여, 목표값의 시의성 문제를 분리해야 한다. |
| end-to-end latency와 품질을 함께 측정할 것 | 논문은 한 데이터셋의 generation time만 보고한다. |

이 조건은 논문이 산업 배포 체크리스트로 제시한 것이 아니라, 방법의 가정과 실험 범위에서 직접 도출한 적용 기준이다.
