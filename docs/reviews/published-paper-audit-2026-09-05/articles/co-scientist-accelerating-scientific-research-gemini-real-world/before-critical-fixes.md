**Paper:** Schmidgall, S., Zhu, X., Shaw, M., Yang, L., Liévin, V., Yang, J., Zhuang, Y., Strother, T., Bijamov, A., Sun, M. W., Palepu, A., Chen, J., Steiner, D., Shreibati, J., Weng, W.-H., Zhao, Y., Hu, X., Zahn, N., Garg, S., … Tu, T. (2026). *Accelerating scientific research with Gemini in the real-world* (arXiv:2608.26701v1, 2026-08-27). arXiv. https://arxiv.org/abs/2608.26701v1

**Abstract:** Schmidgall et al.은 가설 생성에 집중했던 Co-Scientist를 실험 장비 연동, 코드 실행, 원고 작성까지 수행하는 실행 기반 연구 에이전트로 확장한다. 논문은 재료합성, 생물학, 의료 소프트웨어, 논문 자동생성의 네 영역에서 시스템을 평가한다. 신뢰성 모듈을 켠 논문생성 조건에서는 결과 환각이 46%에서 4%로 줄었고, Agent_H는 HealthBench 길이보정 네 조건에서 모두 1위를 기록했다. 다만 MXene 유사 물질의 정체는 확정되지 않았고, Agent_H와 기준선의 호출 예산은 맞춰지지 않았으며, 사람 연구자와 시간·비용을 통제한 비교도 없다. 이 글은 실행·검증 루프를 먼저 재구성한 뒤 네 평가의 결과와 경계를 나누어 살핀다.
...
해석하기 전에 확인할 지점도 있다. 다음 항목은 논문이 직접 인정한 한계가 아니라, 공개된 연구설계와 보고 방식에서 도출한 해설자의 해석이다.
...
여기서부터는 논문이 직접 인정한 한계가 아니라 공개된 설정과 보고 방식에서 도출한 해설자의 해석이다. 83쪽 분량에 본문 표는 두 개뿐이라 주요 결과의 상당 부분이 도판과 부록에 흩어져 있다. 임상 위해의 "though modest"와 HealthBench의 길이보정 조건은 초록에서 빠지고, MXene 유사 물질은 §3.1.2 일부가 초록과 결론보다 더 강하게 표현한다.
