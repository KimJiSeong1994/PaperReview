**Paper:** Edge, D., Trinh, H., Cheng, N., Bradley, J., Chao, A., Mody, A., Truitt, S., Metropolitansky, D., Ness, R. O., & Larson, J. (2024). *From Local to Global: A Graph RAG Approach to Query-Focused Summarization*. arXiv:2404.16130v2. [arXiv](https://arxiv.org/abs/2404.16130) · [Official code](https://github.com/microsoft/graphrag)
...
여기서 중요한 구분이 하나 있다. 현재 Microsoft GraphRAG 구현은 local search와 여러 검색 방식을 제공하지만, 이 논문이 주로 평가한 것은 **community summary를 사용하는 global search**다. 따라서 이 글도 현재 소프트웨어 전체가 아니라 원논문이 검증한 global QFS 경로에 초점을 맞춘다.
...
리뷰용 표기로 indexing을 단순화하면 다음과 같다. 아래 수식은 논문의 원래 표기가 아니라 파이프라인을 설명하기 위한 재구성이다.
...
리뷰용 표기로 level $\ell$의 $k$번째 community report를 다음처럼 쓸 수 있다.
...
### 8.7 논문과 현재 저장소는 같은 시점의 제품이 아니다

검토한 문서는 arXiv v2 preprint다. 공식 저장소는 이후 계속 변경됐다. 저장소도 indexing이 비쌀 수 있고, 제공 코드는 정식 지원 Microsoft 제품이 아니라 방법론 시연이라고 명시한다. 현재 quick start 결과를 논문 실험의 직접 재현으로 간주하면 안 된다.
...
Lewis, P., Perez, E., Piktus, A., Petroni, F., Karpukhin, V., Goyal, N., Küttler, H., Lewis, M., Yih, W.-T., Rocktäschel, T., Riedel, S., & Kiela, D. (2020). Retrieval-augmented generation for knowledge-intensive NLP tasks. *Advances in Neural Information Processing Systems, 33*, 9459–9474. https://proceedings.neurips.cc/paper/2020/hash/6b493230-Abstract.html
