**Paper:** Helbling, A., Bryutkin, A., Martino, M., Chau, D. H., Dehmamy, N., & Strobelt, H. (2026). *Flow reasoning models: Turning flows into efficient recurrent reasoners* (arXiv:2606.29150v3, 2026-09-01). arXiv. https://arxiv.org/abs/2606.29150v3
...
Sudoku의 32.6→99.2가 논문에서 가장 귀속이 분명한 개선이다. 구조와 canonical loss path는 유지하고 carry의 출처만 rollout state로 바꾼 비교이기 때문이다. Maze는 96.2→98.0으로 폭이 작지만 같은 방향이다.
...
### 8.1 가장 강한 근거는 carry 출처의 직접 개입이다

FRM의 가장 설득력 있는 근거는 Table 1의 최고점보다 carry 출처를 바꾼 Table 2의 절제와 Figure 6의 깊이별 진단이 같은 방향을 보인다는 점이다. 구조와 정답 기반 flow 경로, endpoint CE를 유지한 좁은 개입이므로 변화의 귀속이 비교적 분명하다. 모든 carry가 동일한 해를 예측해 각 반복 갱신에 직접 token supervision을 줄 수 있고, rollout을 통과하는 역전파도 필요 없다.
...
평가 과제는 Sudoku, 관계 퍼즐, 유일 경로 maze로 서로 다른 추론 형식을 담지만 모두 exact solution이 정의되는 격자형 구조 문제다. Sudoku와 Maze의 학습 규모는 각각 1,000개 수준이며, 더 큰 모델·자연어 추론·정답이 여러 개인 열린 문제로의 전이는 검증되지 않았다. 논문도 larger model과 less structured task를 후속 방향으로 제시한다.

추론 구성도 완전히 재구성되지 않는다. Euler·SDE·held-time sampler의 이름은 나오지만 의사코드와 과제별 최종 선택이 없다. 각 seed의 best validation 구성을 고르는 과정에는 sampler와 NFE가 포함되므로, 고정 예산에서 동일 결과가 나오는지 확인하려면 구현과 선택 log가 필요하다.

논문은 공식 GitHub 저장소를 연결하지만 2026년 9월 5일 기준 tree에는 README와 assets만 있고 README는 implementation이 추후 제공될 예정이라고 적는다. 따라서 sampler, preprocessing, evaluation harness를 paper-exact하게 재현할 코드는 아직 없다.

### 8.4 실제 적용 전에 확인할 조건

| 조건 | 이유 |
| --- | --- |
| 출력 전체를 반복 수정할 수 있는가. | FRM은 노이즈 제거 예측 전체를 carry로 되먹이는 구조다. |
| correctness 없이 수렴을 판단할 수 있는가. | oracle NFE와 deployable early stopping은 다르다. |
| rollout probability·depth를 별도 검증했는가. | 보고된 FPF는 probability 0.5·depth 16의 묶음이다. |
| 실제 경과시간과 memory cost를 측정했는가. | 논문의 FLOPs는 산술 연산량의 하한이다. |
| 고정된 sampler·NFE에서도 비교했는가. | 핵심 평균은 validation-selected 구성을 사용한다. |
| 열린 출력이나 대규모 task에서도 유지되는가. | 현재 증거는 세 exact-solve 격자 문제에 한정된다. |

이 표는 논문이 배포 checklist로 제시한 것이 아니라, 방법의 가정과 실험 조건에서 직접 도출한 적용 기준이다.
...
가장 직접적인 근거는 FPF 절제와 Figure 6의 깊이별 loss·AUROC 변화다. carry 출처만 바꾼 좁은 개입이어서 평균 향상과 seed variance 감소를 FPF에 귀속하기 쉽다. 반면 계산 효율 주장은 운영점 선택과 FLOP 하한에 의존하고, 수렴은 정확성의 형식 보증이 아니다.
...
- Helbling, A., et al. (2026). *Flow Reasoning Models* [Project repository; implementation pending as of 2026-09-05]. GitHub. https://github.com/helblazer811/Flow-Reasoning-Models
