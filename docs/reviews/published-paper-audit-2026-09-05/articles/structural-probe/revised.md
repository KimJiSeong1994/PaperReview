...
**Paper:** John Hewitt; Christopher D. Manning (2019). "A Structural Probe for Finding Syntax in Word Representations". https://aclanthology.org/N19-1419/ · https://doi.org/10.18653/v1/N19-1419
...
표현 모델은 5.5B 단어로 사전학습된 ELMo, cased BERT-base, cased BERT-large다. 각각 $\mathrm{ELMo}_K$, $\mathrm{BERT}_{\mathrm{base}K}$, $\mathrm{BERT}_{\mathrm{large}K}$로 쓰며 $K$는 은닉층 색인이다. ELMo와 BERT-large의 층은 차원 1024, BERT-base는 768이다. BERT는 subword를 쓰므로, subword 벡터를 gold Penn Treebank 토큰에 정렬해 각 토큰에 subword 표현의 평균을 배정한다. 저자들은 이 평균화가 BERT 성능의 하한을 나타낸다고 명시한다.
...
- $\mathrm{Linear}$: 영어 parse tree가 좌→우 사슬을 이룬다는 가정에서 나오는 트리. 단어 위치를 인코딩하는 모델이라면 이 수준은 넘어야 한다.
- $\mathrm{ELMo0}$: 문맥 정보가 없는 문자 수준 임베딩. 위치 정보조차 없으므로 트리를 전혀 찾지 못해야 한다.
- $\mathrm{Decay0}$: 각 단어에 문장 내 모든 $\mathrm{ELMo0}$ 임베딩의 가중 평균을 배정한다. 가중치는 선형 거리 $d$에 따라 $\tfrac{1}{2^d}$로 지수 감쇠한다.
- $\mathrm{Proj0}$: $\mathrm{ELMo0}$ 임베딩을 ELMo와 같은 차원(1024)의 무작위 초기화 BiLSTM 한 층으로 문맥화한다. 문맥화만으로도 강한 베이스라인이다(Conneau et al., 2018).
...
수치는 test set 기준이다. 베이스라인은 $\mathrm{Linear}$ 48.9 / 0.58 / 2.9 / 0.27, $\mathrm{ELMo0}$ 26.8 / 0.44 / 54.3 / 0.56, $\mathrm{Decay0}$ 51.7 / 0.61 / 54.3 / 0.56, $\mathrm{Proj0}$ 59.8 / 0.73 / 64.4 / 0.75다(UUAS / DSpr / Root% / NSpr 순). 모델은 $\mathrm{ELMo}_1$ 77.0 / 0.83 / 86.5 / 0.87, $\mathrm{BERT}_{\mathrm{base}7}$ 79.8 / 0.85 / 88.0 / 0.87, $\mathrm{BERT}_{\mathrm{large}15}$ 82.5 / 0.86 / 89.4 / 0.88, $\mathrm{BERT}_{\mathrm{large}16}$ 81.7 / 0.87 / 90.1 / 0.89다.

먼저 프로브가 스스로 파싱하지 못한다는 것이 확인된다. $\mathrm{ELMo0}$과 $\mathrm{Decay0}$은 단어의 선형 순서만 담은 우분지 트리 oracle($\mathrm{Linear}$)을 실질적으로 넘지 못한다. $\mathrm{Proj0}$은 $\mathrm{ELMo}_1$의 표현 용량은 전부 갖되 학습은 전혀 하지 않은 무작위 문맥화인데도 베이스라인 중 가장 강하다(UUAS 59.8, DSpr 0.73). 문맥화 자체가 어느 정도 구조를 만들지만, 학습된 모델은 이를 뚜렷이 넘어선다.

![$\mathrm{BERT}_{\mathrm{large}16}$·$\mathrm{ELMo}_1$의 예측 제곱 거리에서 얻은 최소 신장 트리와 최강 베이스라인 $\mathrm{Proj0}$의 비교. 검은 간선은 gold, 파랑은 $\mathrm{BERT}_{\mathrm{large}16}$, 빨강은 $\mathrm{ELMo}_1$, 보라는 $\mathrm{Proj0}$.](/api/blog/figures/structural-probe-fig2-min-spanning-trees.png)
...
모델 사이에서는 $\mathrm{BERT}_{\mathrm{large}} > \mathrm{BERT}_{\mathrm{base}} > \mathrm{ELMo}$가 일관되게 나타난다. 층 사이에도 뚜렷한 차이가 있어, 구문 정보는 중간 층들에서 가장 강하다. 저자들은 이 가설이 ELMo 같은 LSTM을 분석하며 세워졌는데도 self-attention 기반 BERT에 수정 없이 적용된다는 점을 짚는다.
...
프로브가 선형(저용량)이라는 점과 베이스라인이 크게 뒤진다는 점은 "프로브가 스스로 파싱해 높은 점수를 낸 것 아니냐"는 우려를 상당 부분 막는다. 그러나 완전히 해소하지는 않는다. $\mathrm{Proj0}$은 무작위 문맥화인데도 DSpr 0.73을 내는데, 학습된 모델은 0.83–0.87이다. 거리 상관 지표에서 무작위 문맥화와 학습 모델의 간극은 크지 않다(UUAS 간극이 더 크다). 즉 어떤 지표에서는 문맥화 자체가 이미 구조 대부분을 만든다.
