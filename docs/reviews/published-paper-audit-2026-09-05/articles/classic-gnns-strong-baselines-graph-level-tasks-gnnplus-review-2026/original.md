...
원논문 §5.2와 Tables 5–6이 여섯 기법을 하나씩 제거한 결과다. 논문의 요약 주장은 각 모듈이 불가결하며 어느 하나를 빼도 성능이 저하된다는 것이지만(§5.2, 번역), 표를 뜯어보면 실제 그림은 데이터셋마다 취약한 기법이 다르다는 쪽에 가깝다(논문 외 해석).
...
여기서 이 논문의 서술 하나가 표와 어긋난다. "모든 모듈이 불가결(indispensable)"이라는 문장과, 하이퍼파라미터 표(Tables 8–13)에서 여러 데이터셋이 엣지·FFN·위치 인코딩·잔차를 애초에 끄는(False) 사실이 충돌한다. 더 정확한 서술은 각 기법이 일부 데이터셋에서 중요하고 최적 조합은 데이터셋마다 다르다는 것이다(논문 외 비판). 그리고 이 관찰이 5장의 핵심이다. GNN+가 이겼다기보다, 여섯 스위치의 데이터셋별 서치 결과가 GT의 고정 보고값을 넘겼다고 보는 편이 실상에 맞다.

## 6. 주의해서 읽을 점

### 6.1 "고전 GNN"이라는 이름이 가리는 것

가장 약한 지점은 명명이다. GNN+가 얹은 여섯 기법 중 최소 셋(FFN·위치 인코딩·엣지 인코딩 방식)은 GT/Transformer에서 온 부품이고, 논문 스스로 그 출처를 밝힌다(§3.1·§3.5·§3.6). 구조적으로 GNN+는 GraphGPS의 로컬 모듈 레시피에서 전역 어텐션만 뺀 것에 가깝다. 그렇다면 이 논문의 방어 가능한 진짜 주장은 "전역 어텐션이 그래프 수준 과제에 불필요하다"이고, 이것은 그 자체로 따져 볼 만한 명제다. 문제는 이를 "simple/classic GNN의 승리"로 포장한다는 점이다(논문 외 비판). FFN·위치 인코딩·10~20층·수백만에서 수천만 파라미터를 얹은 모델을 "classic"이라 부르는 것은 수사적 과장이고, 그 표제만으로는 실제로 이긴 것이 집계 규칙인지 부착된 부품인지 알 수 없다. 그리고 5장의 ablation은 부품 쪽 기여가 결정적임을 보여준다.

### 6.2 튜닝과 파라미터 예산의 비대칭

§4의 공정성 방어에도 세 빈틈이 남는다(논문 외 비판). 첫째, "GT를 재학습했으나 원논문을 못 넘었다"는 주장은 재현 로그 없이는 검증 불가능하고, GT 재학습이 저자의 재현 실패인지 GT의 진짜 천장인지 구분되지 않는다. DW2V·DBE 리뷰에서 반복 지적한, 제안 모델은 집중 튜닝하고 비교군은 저자 보고값을 재사용하는 비대칭의 그래프 수준 판본이다. 둘째, 탐색 공간이 같다는 것과 탐색 노력이 같다는 것은 다르다. GNN+는 데이터셋마다 층수·차원·여섯 스위치·풀링을 개별 최적화하지만, GT는 그런 데이터셋별 재탐색 없이 고정 보고값으로 들어온다. 셋째가 가장 중대하다. 소규모 벤치마크는 파라미터 예산을 맞췄지만 OGB에는 제약이 없어서, GNN+의 OGB 모델은 GCN+ molpcba 13.3M, GatedGCN+ code2 29.9M까지 커진다. 즉 OGB의 1위 주장은 동일 파라미터 예산 하의 승리가 아니고, 논문은 이 불일치를 언급하지 않는다.

### 6.3 통계적으로 구분되지 않는 순위

전 표가 5-시드 표준편차를 보고한다는 점에서 평가 위생은 이전에 리뷰한 논문들보다 낫다. 그러나 통계 검정은 없다. 순위가 점추정 평균만으로 매겨지는데, 근소차 순위들은 표준편차 안에 있다. PATTERN 3위 경합(GatedGCN+ 87.029 vs GCN+ 87.021 vs GRPE 87.020)은 0.01 수준 클러스터이고, code2 3위는 GraphGPS와 0.0002 차, molhiv 1위는 0.0005 내 3파전이다. "8개 1위"의 상당수가 이런 노이즈 수준 차이라, 순위를 강하게 받아들이면 안 된다(논문 외 해석).
