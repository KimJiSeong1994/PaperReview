...
**Paper:** Bryan Perozzi; Rami Al-Rfou; Steven Skiena (2014). "DeepWalk: Online Learning of Social Representations". https://arxiv.org/abs/1403.6652 · https://doi.org/10.1145/2623330.2623732 · arXiv:1403.6652
...
**Abstract:** 이 연구는 random walk 기반 node embedding의 출발점인 DeepWalk 모델을 설명한다. 논문의 아이디어는 하나의 유비다: scale-free 그래프 위 짧은 random walk에서 정점이 등장하는 빈도는 자연어의 단어 빈도와 같은 power law를 따르므로, walk를 문장으로, 정점을 단어로 취급하면 언어 모델(SkipGram)을 그대로 그래프에 이식할 수 있다. 학습된 표현은 라벨과 독립적인 비지도 산출물이라 여러 분류 과제에 재사용되고, 라벨이 희소한 구간에서 당시 baseline을 앞섰으며, spectral 방법이 실행되지 않는 규모(YouTube, 114만 정점)까지 확장됐다. 한계는 7장에서 정리한다.
...
### 7.4 GNN 계열이 넘어선 두 한계의 원점

논문의 online learning은 SGD에 의한 순차 갱신을 뜻한다. 보고된 실험에는 미리 생성한 랜덤워크를 사용했으며, §4.4.1이 제안한 별도 streaming 변형의 성능은 이 실험에서 평가하지 않았다.

논문이 online이라 부르는 실체는 SGD의 순차 갱신(§4.2.3), 부분 재학습 가능성(§3.1), 그리고 지역 정보만 쓰는 설계(§7의 차별점)다. 그러나 본 실험의 학습은 배치다: 시작 전에 전체 V로 트리를 만들고, 전체 정점을 셔플해 γ=80회 pass를 돌며, 학습률 스케줄도 전체 규모를 안다. 진짜 streaming은 §4.4.1의 조건부 스케치("if"가 두 번 들어간다)로만 존재하고 실험되지 않았다. streaming 성능은 이 실험의 평가 범위에 포함되지 않는다.
