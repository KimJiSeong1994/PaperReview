...
**Paper:** Zengqing Wu; Run Peng; Takayuki Ito; Makoto Onizuka; Chuan Xiao (2025). "Position: LLM-Based Social Simulations Require a Boundary". https://arxiv.org/abs/2506.19806 · arXiv:2506.19806v3

**Abstract:** 이 포지션 페이퍼는 LLM 사회시뮬레이션의 타당성을 평균 정렬과 행동 분산의 두 축으로 논의한다. LLM이 모집단의 전형적 반응을 재현하더라도 실제 인간의 이질성을 충분히 표현하지 못할 수 있다는 문제를 average persona 개념으로 설명한다. 시뮬레이션 연구 21편의 검증 관행을 코딩하고, 같은 현상에 대해 상반된 결과가 보고된 사례를 분석한다. 이를 바탕으로 연구 질문이 요구하는 이질성에 맞춰 검증 수준을 정하고, 평균과 함께 분산을 보고하며, 충분히 검증되지 않은 결과의 적용 범위를 제한하도록 권고한다.
...
논문은 맞논문인 Anthis et al. (2025)를 §4.2에서는 '다양성 부족'의 근거로, Appendix A.3에서는 '인간 근접 성능'의 근거로도 인용한다. 상대 진영 논문을 자기 논거의 부분 증거로 흡수하는 방식이다.

## 6. 결과의 해석 범위

### 6.1 선정과 코딩의 범위

21편의 검증 관행을 분류한 결과는 해당 표본과 저자들의 코딩 판단에 조건부다. 논문은 Appendix D.1에서 분류에 판단이 개입함을 인정한다. 이를 전체 LLM 사회시뮬레이션 연구의 확률표본으로 해석할 수는 없다.


### 6.2 경험 근거는 모델 세대에 묶여 있다

"LLM은 인간보다 분산이 낮다"의 실체는 2023–2025년 GPT-3.5/4 세대의 관찰이다(Table 2의 모델 열). 논문 스스로 이 시효를 §5.2에 적어 두었다. Llama 3가 이전 세대와 현저히 다른 행동을 보이며, "빠르게 진화하는 모델 아키텍처가 기존 행동 특성화를 무효화할 수 있다"는 것이다. 모델이 바뀌면 무너질 주장(Figure 2의 저분산, RLHF 모델의 진보 편향, 협력 역설의 개별 결과들)과 살아남을 주장(분산-평균 분해 틀, 검증 깊이 매칭 방법론, 회고 검증의 데이터 누출 문제)을 구분해서 읽어야 한다. Appendix F의 모순 감사 권고는 사실상 "우리의 경험적 전제도 세대마다 재검증하라"는 자인이다.
