...
**Paper:** Manuel Faysse; Hugues Sibille; Tony Wu; Bilel Omrani; Gautier Viaud; Céline Hudelot; Pierre Colombo (2025). "ColPali: Efficient Document Retrieval with Vision Language Models". https://arxiv.org/abs/2407.01449 · arXiv:2407.01449

**Abstract:** ColPali는 시각적으로 복잡한 문서의 페이지 이미지를 직접 임베딩하고 늦은 상호작용으로 검색 점수를 계산한다. 논문은 페이지 단위 문서 검색 벤치마크 ViDoRe와 ColPali 모델을 함께 제안한다. OCR·레이아웃 탐지·청킹을 거치는 적재 과정 대신 시각언어 모델의 다중 벡터 표현을 사용하며, 여러 문서 유형에서 높은 검색 성능을 보고한다. 데이터 구성, 질의 생성, 지연과 저장 비용 및 데이터셋별 편차가 성능의 해석 범위를 정한다.
...
*그림 1. Faysse et al. (2025), Figure 1. 위는 표준 검색으로, 오프라인에서 OCR과 레이아웃 탐지를 거쳐 청크를 만든다(쪽당 7.22초). 아래가 ColPali로, 페이지 이미지를 시각 인코더와 LLM에 한 번 통과시켜 다중 벡터를 얻는다(쪽당 0.39초). 온라인은 양쪽 다 MaxSim 계열 점수 계산이다. 인용. [arXiv:2407.01449](https://arxiv.org/abs/2407.01449).*
...
*그림 2. Faysse et al. (2025), Figure 2. 오프라인 색인의 쪽당 지연. 막대에 인쇄된 값은 합계 7.22초·0.12초·0.39초이고, 구성요소 값(레이아웃 탐지 0.81초, OCR 2.67초, 캡셔닝 3.71초, 페이지 인코딩 0.03초)은 부록 Table 5 출처다. 인용. [arXiv:2407.01449](https://arxiv.org/abs/2407.01449).*
...
*그림 3. Faysse et al. (2025), Figure 3. 왼쪽은 토큰 풀링으로 벡터 수를 줄일 때의 상대 성능 저하, 오른쪽은 질의 토큰에 대한 패치별 유사도 지도다. 인용. [arXiv:2407.01449](https://arxiv.org/abs/2407.01449).*
