# SEED 심층 분석: sparse reward를 hindsight skill로 촘촘하게 만드는 Agentic RL
...
공식 저장소는 MIT license로 공개됐고, ALFWorld·WebShop·Search·Sokoban·EZPoints의 SFT/RL entrypoint와 OPD loss unit test를 제공한다. 이 점은 강점이다. 반면 2026년 7월 20일 확인한 public defaults에는 논문과 맞지 않는 부분이 있다.

| 항목 | 논문 | 공개 기본 설정 |
| --- | --- | --- |
| ALFWorld·WebShop training length | 150 policy updates | launcher 기본 160 epoch |
| Search KL coefficient | 0.01 | common config 0.001 |
| ALFWorld seen evaluation | 140 task | default validation size 128 |
| Search evaluation | 51,713 question | default validation size 512 |
| Teacher re-scoring | 각 optimization minibatch에서 두 branch를 current $\pi_\theta$로 평가 | actor update 전에 skill-conditioned log-probability를 계산·cache |

이는 코드가 재현 불가능하다는 뜻은 아니다. 다만 paper-exact reproduction을 위해서는 README command를 그대로 실행하는 것보다 논문 Table 3·5에 맞춰 설정을 고정하고, 평가 전체 split을 별도로 지정해야 한다. 공개 checkpoint도 현재 README에서는 ALFWorld 3B 하나만 확인된다.
