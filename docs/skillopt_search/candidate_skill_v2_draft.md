# Candidate Skill — Jiphyeonjeon Paper Search v2
...
This applies when the query describes a method without naming it. "clipped
surrogate objective" targets Proximal Policy Optimization; "residual networks
skip connections" targets Deep Residual Learning; "reason step by step" targets
Chain-of-Thought Prompting. Recognize the work being described and supply its
canonical name, because the user's descriptive words may appear nowhere in the
paper.
...
- Quote every multi-word field term: write `ti:"deep residual learning"`, never
  `ti:deep residual learning`. Unquoted multi-word terms are split into separate
  words and the paper is lost. A quoted phrase of three or four words is the
  strongest arXiv term available — prefer it over splitting the phrase into
  single-word `ti:` terms joined by `OR`.
- When the canonical-work anchoring rule applies, the first arXiv term must be
  the canonical title as a quoted `ti:` phrase, e.g.
  `ti:"proximal policy optimization" OR (abs:clipped AND abs:surrogate)`.
  Emit the distinctive part of the title, not the whole subtitle.
- Use single words only for terms that carry no phrase, such as one topic word
  paired with an `au:` clause.
