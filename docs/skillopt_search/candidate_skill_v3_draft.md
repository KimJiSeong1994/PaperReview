# Candidate Skill — Jiphyeonjeon Paper Search v3
...
- Keep each decomposed field term to a single word. An unquoted multi-word
  `ti:` term is split into separate words and matches poorly.
- When the canonical-work anchoring rule applies, add the canonical title as
  **one** quoted `ti:` phrase placed inside the same OR group as the decomposed
  terms — never in place of them:
  `(ti:"proximal policy optimization" OR ti:clipped OR ti:surrogate OR ti:policy) OR (abs:clipped AND abs:surrogate)`
  The quoted phrase wins when the guess at the title is right; the decomposed
  terms still return candidates when it is wrong.
- Emit at most one quoted phrase per query, and never let quoted phrases be the
  only `ti:` terms. Several narrow quoted phrases shrink the result set until
  nothing matches at all, which is worse than a loose query.
