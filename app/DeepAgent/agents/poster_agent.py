...
"""
    ...
    def _build_visualizations_html(self, content, autofigure_svgs: list = None) -> str:
        ...
        Gemini 포스터에 이미 SVG가 포함되어 있으면 추가하지 않는다.
        AutoFigure SVG가 있으면 우선 삽입한다.
        """
        parts = []

        # AutoFigure SVGs (외부 서비스에서 생성된 고품질 SVG)
        if autofigure_svgs:
            for fig in autofigure_svgs[:3]:
                title = self._escape_html(fig.get("paper_title", ""))
        ...
        # 보조 파이프라인 SVG (Gemini SVG가 부족할 때)
            ...
            if metrics:
                ...
                for m in metrics[:6]:
                    if isinstance(m, dict):
                        try:
                            values.append(float(m.get('value', 0)))
                        except (ValueError, TypeError):
                            continue
                if labels and values:
                    max_v = max(values) if values else 1
                    norm = [v / max_v for v in values] if max_v > 1 else values
                    svg = self.visual_agent.generate_bar_chart({'labels': labels, 'values': norm})
                    parts.append(f'''<div style="background:#f8fafc; border-radius:10px; padding:16px; border:1px solid #e2e8f0; margin-bottom:16px;">
                        {svg}
    ...
    def _build_gemini_prompt(self, content, layout, report_summary: str, num_papers: int, figures: list = None) -> str:
        """Gemini용 포스터 생성 프롬프트 — SVG 인라인 생성 포함"""

        return f"""당신은 NeurIPS/ICML 학회 포스터 디자이너입니다. 완전한 HTML 포스터를 생성하세요.

## 콘텐츠

**제목**: {content.title}
**부제목**: {content.subtitle} ({num_papers}편 논문 분석)

**초록**: {content.abstract[:800]}
**배경**: {content.motivation[:600]}
**기여**: {chr(10).join(f"• {c}" for c in content.contributions[:6])}
**방법론**: {content.methodology[:1200]}
**주요 발견**: {chr(10).join(f"- {f}" for f in content.key_findings[:6])}
...
3단 CSS Grid (`grid-template-columns: 1fr 1.2fr 1fr`, min-width 1600px)

**좌측**: Motivation, Abstract, Methodology
**중앙**: **SVG 아키텍처 다이어그램** (필수), 비교 테이블, 결과 차트
**우측**: Key Findings, Conclusion, Keywords

## SVG 시각화 (필수 — 반드시 포함)

**1. 아키텍처/파이프라인 다이어그램** (중앙 컬럼, 필수):
방법론 섹션의 핵심 프로세스를 SVG 플로우차트로 표현하세요.
```
<svg viewBox="0 0 800 200" style="width:100%">
  ...
  <!-- 둥근 사각형 박스 + 화살표 연결 -->
  <rect rx="12" fill="#dbeafe" stroke="#2563eb" stroke-width="2"/>
  <text font-weight="bold" fill="#2563eb">단계명</text>
  <line stroke="#64748b" marker-end="url(#arrow)"/>
</svg>
```
- 방법론에서 3-6개 핵심 단계를 추출하여 박스+화살표로 연결
- 각 박스에 단계명과 설명을 포함
- viewBox를 적절히 설정 (최소 600x150)

**2. 결과 바 차트** (정량 데이터가 있으면):
```
<svg viewBox="0 0 400 250" style="width:100%">
  <line stroke="#cbd5e1"/> <!-- 축 -->
  <rect fill="#2563eb" opacity="0.8"/> <!-- 막대 -->
  <text fill="#334155">레이블</text>
</svg>
```

## 디자인 규칙

- 색상: `--primary: #2563eb; --secondary: #1e293b; --accent: #f59e0b; --bg: #f8fafc`
- 폰트: Inter, Noto Sans KR (Google Fonts import)
- 제목: 3rem/900, 섹션: 1.3rem/700, 본문: 1rem/1.6 line-height
- 각 섹션: `class="section-box"`, 배경 #f8fafc, 테두리 #e2e8f0, border-radius 10px
- 포스터 컨테이너에 `box-shadow: 0 4px 24px rgba(0,0,0,0.08)`

## 출력

<!DOCTYPE html>로 시작하는 완전한 HTML을 출력하세요.
- `<head>`에 `<style>` 태그로 모든 CSS 포함
- SVG를 HTML 안에 인라인으로 직접 포함 (외부 파일 참조 금지)
- 설명이나 마크다운 코드블록 없이 HTML만 출력"""
