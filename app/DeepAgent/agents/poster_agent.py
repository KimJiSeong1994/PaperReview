...
"""
    ...
    def _generate_with_gemini(self, content, layout, report_content: str, num_papers: int,
        ...
        """Gemini로 레이아웃 HTML 생성 후 SVG/Figure를 후삽입한다."""
        ...
        try:
            ...
            # Phase B: 논문 Figure 후삽입
            fig_html = self._build_figures_html(figures)
            poster_html = poster_html.replace("<!-- FIGURES_PLACEHOLDER -->", fig_html)

            # 플레이스홀더가 남아있으면 (Gemini가 안 넣었을 경우) body 끝에 삽입
            if viz_html and "<!-- VISUALIZATIONS_PLACEHOLDER -->" not in poster_html and viz_html not in poster_html:
                poster_html = poster_html.replace("</body>", f"{viz_html}</body>")
            if fig_html and "<!-- FIGURES_PLACEHOLDER -->" not in poster_html and fig_html not in poster_html:
...
"""
    ...
    def _build_visualizations_html(self, content, autofigure_svgs: list = None) -> str:
        ...
        # 기존 visual_agent SVG 차트 (AutoFigure가 없거나 부족할 때)
        ...
        if len(parts) < 2:
            ...
            if pipeline_steps:
                ...
                parts.append(f'<div style="margin-bottom:16px;">{svg}</div>')

        if len(parts) < 3:
            # Bar chart from metrics
            quant = viz_data.get('quantitative', {})
