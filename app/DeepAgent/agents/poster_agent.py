...
"""
    ...
    def _build_gemini_prompt(self, content, layout, report_summary: str, num_papers: int, figures: list = None) -> str:
        ...
        if figures:
            ...
            for i, fig in enumerate(figures[:4]):
                try:
                    paper_title = str(getattr(fig, 'paper_title', '') or '')[:50]
                    caption = str(getattr(fig, 'caption', '') or '')
                    description = str(getattr(fig, 'description', '') or '')[:200]
                    try:
                        relevance_str = f"{float(getattr(fig, 'relevance_score', 0)):.1f}"
                    except (ValueError, TypeError):
                        relevance_str = "N/A"
                    width = getattr(fig, 'width', 'N/A')
                    height = getattr(fig, 'height', 'N/A')
                    figures_info.append(f"""
**삽도 {i+1}** (출처: {paper_title}):
- 캡션: {caption}
- 설명: {description}
- 핵심도: {relevance_str}/1.0
