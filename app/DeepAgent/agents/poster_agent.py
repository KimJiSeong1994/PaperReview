...
"""
    ...
    def _inject_visuals_into_poster(self, poster_html: str, content, figures, autofigure_svgs) -> str:
        """포스터 HTML에 시각화 + Figure를 확정적으로 삽입한다.

        Gemini가 플레이스홀더를 넣든 안 넣든, 무조건 삽입을 보장한다.
        """
        ...
        # 2차: 플레이스홀더가 없었으면 </body> 앞에 무조건 삽입
        if viz_html and viz_html not in poster_html:
            poster_html = poster_html.replace("</body>", f"\n{viz_html}\n</body>")
        if fig_html and fig_html not in poster_html:
            poster_html = poster_html.replace("</body>", f"\n{fig_html}\n</body>")
    ...
    def _generate_with_gemini(self, content, layout, report_content: str, num_papers: int,
        ...
        """Gemini로 레이아웃 HTML 생성 후 SVG/Figure를 확정적으로 삽입한다."""
...
"""
    ...
    def _generate_architecture_image(self, content) -> str:
        """Gemini 이미지 생성으로 논문 아키텍처 다이어그램을 생성한다.

        Returns:
            base64 인코딩된 PNG 이미지 문자열, 실패 시 빈 문자열
        """
        if not self.llm:
            return ""
        ...
        # 방법론에서 핵심 내용 추출
        method_desc = methodology[:800]
        if paper_analyses:
            first_paper = paper_analyses[0]
            paper_method = (first_paper.get('methodology', '') or '')[:500]
            paper_title = first_paper.get('title', '')
            if paper_method:
                method_desc = f"{paper_title}: {paper_method}"

        if not method_desc.strip():
            return ""

        prompt = f"""Generate a professional academic architecture diagram image for a research paper.

The diagram should visualize this methodology:
{method_desc}

Requirements:
- Clean, professional style suitable for a conference poster
- Use boxes, arrows, and labels to show the pipeline/architecture
- Light background, clear typography
- Include component names and data flow arrows
- Academic/scientific visual style (like diagrams in NeurIPS/ICML papers)
...
- No decorative elements, focus on clarity"""
        ...
        try:
            ...
            print(f"[PosterAgent] 아키텍처 이미지 생성 실패 (SVG fallback 사용): {e}")

        return ""

    def _build_visualizations_html(self, content, autofigure_svgs: list = None) -> str:
        """SVG 시각화를 조합하여 HTML 블록을 생성한다.

        항상 최소 1개 이상의 시각화를 보장한다.
        우선순위: AutoFigure SVG > Gemini 아키텍처 이미지 > 파이프라인 SVG > 바차트/레이더
        """
        parts = []
        ...
        # 2순위: Gemini 이미지 생성으로 아키텍처 다이어그램
        if len(parts) < 1:
            arch_b64 = self._generate_architecture_image(content)
            if arch_b64:
                parts.append(f'''<div style="background:#f8fafc; border-radius:10px; padding:16px; border:1px solid #e2e8f0; margin-bottom:16px; text-align:center;">
                    <img src="data:image/png;base64,{arch_b64}" alt="Architecture Diagram" style="width:100%; max-width:800px; height:auto; border-radius:8px;" />
                    <p style="font-size:0.85rem; color:#64748b; text-align:center; margin:8px 0 0;">Research Architecture Overview</p>
                </div>''')

        # 3순위: 파이프라인 SVG (methodology에서 추출)
        viz_data = getattr(content, 'visualization_data', {}) or {}

        if len(parts) < 2:
            pipeline_steps = viz_data.get('pipeline_steps', [])
            if not pipeline_steps:
                # methodology에서 직접 추출 시도
                methodology = getattr(content, 'methodology', '') or ''
                if methodology:
                    pipeline_steps = self.visual_agent._parse_methodology_steps(methodology)
            if pipeline_steps:
                svg = self.visual_agent.generate_pipeline_diagram(pipeline_steps)
                parts.append(f'''<div style="background:#f8fafc; border-radius:10px; padding:16px; border:1px solid #e2e8f0; margin-bottom:16px;">
                    {svg}
                    <p style="font-size:0.85rem; color:#64748b; text-align:center; margin:8px 0 0;">Research Pipeline</p>
                </div>''')

        # 4순위: 바 차트 (정량 데이터)
        if len(parts) < 3:
            quant = viz_data.get('quantitative', {})
            metrics = quant.get('metrics', []) if isinstance(quant, dict) else []
            if metrics:
                labels = [m.get('name', '')[:15] for m in metrics[:6] if isinstance(m, dict)]
                values = []
                for m in metrics[:6]:
                ...
                if labels and values:
                    ...
                    norm_values = [v / max_v for v in values] if max_v > 1 else values
                    svg = self.visual_agent.generate_bar_chart({'labels': labels, 'values': norm_values})
                    parts.append(f'''<div style="background:#f8fafc; border-radius:10px; padding:16px; border:1px solid #e2e8f0; margin-bottom:16px;">
                        {svg}
                        <p style="font-size:0.85rem; color:#64748b; text-align:center; margin:8px 0 0;">Key Metrics</p>
                    </div>''')

        # 5순위: 레이더 차트 (논문 분석 데이터가 있으면)
        if len(parts) < 3:
            paper_results = viz_data.get('paper_results', [])
            if paper_results and len(paper_results) >= 2:
                dims = []
                vals = []
                for pr in paper_results[:6]:
                    title = (pr.get('paper_title', '') or '')[:20]
                    pr_metrics = pr.get('metrics', [])
                    if pr_metrics and isinstance(pr_metrics[0], dict):
                        try:
                            v = float(pr_metrics[0].get('value', 0))
                            dims.append(title)
                            max_v = max(v, 1)
                            vals.append(v / max_v if max_v > 1 else v)
                        except (ValueError, TypeError):
                            continue
                if len(dims) >= 3:
                    svg = self.visual_agent.generate_radar_chart({'dimensions': dims, 'values': vals})
                    parts.append(f'''<div style="background:#f8fafc; border-radius:10px; padding:16px; border:1px solid #e2e8f0; margin-bottom:16px;">
                        {svg}
                        <p style="font-size:0.85rem; color:#64748b; text-align:center; margin:8px 0 0;">Paper Comparison</p>
                    </div>''')

        # 6순위: 최소 보장 — 논문 비교 카드 SVG (다른 시각화가 모두 실패했을 때)
        if not parts:
            paper_titles = getattr(content, 'paper_titles', []) or []
            if paper_titles:
                svg = self.visual_agent._generate_paper_comparison_svg(paper_titles[:6])
                parts.append(f'''<div style="background:#f8fafc; border-radius:10px; padding:16px; border:1px solid #e2e8f0; margin-bottom:16px;">
                    {svg}
                    <p style="font-size:0.85rem; color:#64748b; text-align:center; margin:8px 0 0;">Analyzed Papers</p>
                </div>''')
    ...
    def _build_gemini_prompt(self, content, layout, report_summary: str, num_papers: int, figures: list = None) -> str:
        """Gemini용 슬림 프롬프트 — HTML 레이아웃만 생성 (SVG는 후삽입)"""

        return f"""당신은 NeurIPS/ICML 수준의 학술 포스터 HTML 레이아웃 전문가입니다.

## 역할
텍스트 콘텐츠를 배치하는 HTML/CSS 레이아웃만 생성하세요.
**SVG 차트나 이미지는 생성하지 마세요** — 별도로 삽입됩니다.
...
초록: {content.abstract[:800]}

배경: {content.motivation[:600]}

기여:
{chr(10).join(f"• {c}" for c in content.contributions[:6])}

방법론: {content.methodology[:1000]}

주요 발견:
{chr(10).join(f"- {f}" for f in content.key_findings[:6])}

결론: {content.conclusion[:600]}
...
1. **3단 CSS Grid**: `grid-template-columns: 1fr 1.2fr 1fr`
2. **최소 너비**: 1600px
3. **색상**: Primary #2563eb, Secondary #1e293b, Accent #f59e0b, BG #f8fafc
4. **폰트**: Inter, Noto Sans KR (Google Fonts import)
5. **타이포그래피**: 제목 3rem bold, 섹션 1.3rem semibold, 본문 1rem

## 구조 (3단 그리드)

**좌측 컬럼**: Motivation → Abstract → Methodology
**중앙 컬럼**:
  - `<!-- VISUALIZATIONS_PLACEHOLDER -->` (시각화가 여기에 삽입됩니다)
  - 비교 분석 테이블
  - `<!-- FIGURES_PLACEHOLDER -->` (논문 삽도가 여기에 삽입됩니다)
**우측 컬럼**: Key Findings → Conclusion → Keywords

## 필수

- 완전한 HTML (DOCTYPE, html, head with style, body)
- CSS Custom Properties (:root)
- 각 섹션은 class="section-box"로 감싸기
- 중앙 컬럼에 반드시 `<!-- VISUALIZATIONS_PLACEHOLDER -->`와 `<!-- FIGURES_PLACEHOLDER -->` HTML 주석을 포함하세요
- SVG, chart, 이미지를 직접 생성하지 마세요
- 텍스트 내용을 정확히 반영하세요

HTML만 출력하세요. 설명이나 마크다운 코드블록 없이 <!DOCTYPE html>로 시작하세요."""
