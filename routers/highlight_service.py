...
# Backward-compatible alias — some callers and the verification script refer
# to the system prompt without the ``_PROMPT`` suffix.
AUTO_HIGHLIGHT_SYSTEM = AUTO_HIGHLIGHT_SYSTEM_PROMPT

# Fixed user-message prefix for the auto-highlight call. Kept byte-stable so
# it can be reused by OpenAI's automatic prompt cache and by the local
# split-key cache in :mod:`routers.llm_cache`. All per-call variable data
# (topic title, report body) is appended AFTER this prefix.
AUTO_HIGHLIGHT_USER_PREFIX = """\
아래의 지시사항을 엄격히 준수하여, 제공된 리포트에서 핵심 하이라이트를
추출해 주세요.

시스템 프롬프트에 명시된 9개 카테고리 체계와 JSON 출력 스키마를 따르세요.
원문은 한 글자도 변경하지 말고, 섹션 정보를 section 필드에 기록하며,
reviewer_comment는 3단계 구조(평가 판단 → 근거/반론 → 건설적 제안)를
반드시 지키세요.
...
def generate_highlights(report: str, query: str, title: str, client) -> list[dict]:
    ...
    # Variable tail: per-call data goes LAST so the fixed prefix (system +
    # AUTO_HIGHLIGHT_USER_PREFIX) stays cache-stable.
    topic_suffix = (
        f"[연구 주제: {title or query}]\n\n" if (query or title) else ""
    )
    ...
    # Check LLM cache first (split-key aware)
    cached = get_cached(
        AUTO_HIGHLIGHT_SYSTEM_PROMPT,
        user_prompt,
        model,
        temperature,
        fixed_prefix=AUTO_HIGHLIGHT_USER_PREFIX,
    )
...
# Backward-compatible alias — matches the name used by the Week 0
# verification script.
PDF_HIGHLIGHT_SYSTEM = PDF_HIGHLIGHT_SYSTEM_PROMPT

# Fixed user-message prefix for PDF highlight extraction. Immutable across
# calls so OpenAI's automatic prompt cache (and the local split-key cache)
# can reuse it. The variable target-count and paper body are appended AFTER
# this prefix.
PDF_HIGHLIGHT_USER_PREFIX = """\
Follow the system prompt exactly. First detect the paper type, then apply
the matching critical lens, then extract highlights using the 11-category
scheme and the JSON schema defined above.

Runtime reminders:
- Copy highlight text verbatim from the paper.
- Record the source section under `section`.
- Skip any text that appears inside a References / Bibliography block.
- Keep commentary fields in the paper's primary language.
...
def generate_pdf_highlights(
    ...
    # Step 4: Build prompt — fixed prefix first, per-call variable data last
    # so the cache-stable segment can be reused by OpenAI prompt caching.
    topic_suffix = f"[Paper Title: {title}]\n\n" if title else ""
    variable_body = (
        f"{topic_suffix}"
    ...
    # Step 5: Check LLM cache (split-key aware)
    cached = get_cached(
        PDF_HIGHLIGHT_SYSTEM_PROMPT,
        user_prompt,
        model,
        temperature,
        fixed_prefix=PDF_HIGHLIGHT_USER_PREFIX,
    )
    if cached is not None:
