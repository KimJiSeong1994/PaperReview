...
# 제거하면서 이 값을 정하려고 라벨 벤치마크에 실제 랭커를 돌렸다.
#
# 측정 이력을 남겨둔다. 세 번 재보면서 두 번 뒤집혔기 때문이다:
#
#   질의 8개          0.0 이 최선, 가중치가 커질수록 단조 악화
#   질의 24개         2.0 이 최선 — 정반대
#   질의 24개(편향제거) 차이 없음 (nDCG@10 0.2866~0.2979, 폭 0.011)
#
# 두 번째의 역전은 내가 추가한 질의가 정답 논문 제목을 거의 그대로 풀어쓴
# 형태여서 생긴 것이었다("adam stochastic optimization method" →
# "Adam: A Method for Stochastic Optimization"). cross-encoder는 바로 그런
# 문구 재진술에 강하다. 제목 내용어 커버리지 0.6 이상인 질의를 규칙대로
# 다시 쓰자 효과가 사라졌다.
#
# 결론: 이 벤치마크는 가중치를 판정할 해상도가 없다. 어떤 값이든 nDCG 차이는
# 0.011 안이고, Recall@10만 낮은 가중치를 약간 선호한다. 그래서 0.0을 유지한다
# — 품질이 측정 가능하게 나빠지지 않으면서 검색당 cross-encoder 추론 비용이
# 사라지는 값이기 때문이다. 성능이 근거이지 품질 우위가 근거가 아니다.
#
# 이 값을 제대로 정하려면 클릭 데이터가 필요하다. 오프라인 스윕은
# cross_encoder_weight 인자로 언제든 재측정할 수 있다.
