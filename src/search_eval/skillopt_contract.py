...
_ALLOWED_CONFIDENCE_GATE = {"mode": "production_equivalent", "minimum_confidence": _MINIMUM_QUERY_ANALYSIS_CONFIDENCE}
_ALLOWED_OVERLAP_GATE = {"mode": "production_equivalent", "minimum_overlap": 0.5, "max_length_ratio": 2.0}
_ALLOWED_BUDGET_POLICY = {"mode": "generous_fixed_offline_budget", "low_budget_slice": "separate_guardrail_eval"}
_ALLOWED_CONTROLLED_ROLLOUT_GATE = {"state": "shadow_only", "max_rollout_fraction": 0.0, "requires_manual_approval": True}
...
_PII_PATTERNS = (
    ...
    re.compile(r"\b(?:raw user logs?|raw logs|raw user identifiers?|private rejected query|private quer(?:y|ies)|user_id|session_id)\b", re.IGNORECASE),
)
...
def load_json(path: str | Path) -> dict[str, Any]:
    ...
    data = json.loads(
        ...
        parse_constant=lambda constant: (_ for _ in ()).throw(ValidationError(f"non-finite JSON value {constant}")),
    )
...
def validate_dataset_contract(dataset: Mapping[str, Any]) -> None:
    ...
    if not isinstance(guardrail_metrics, Sequence) or isinstance(guardrail_metrics, (str, bytes)):
        raise ValidationError("dataset.guardrail_metrics must be a list")
    if any(not isinstance(metric, str) or not metric.strip() for metric in guardrail_metrics):
        raise ValidationError("dataset.guardrail_metrics must contain non-empty strings")
    ...
    required_guardrail_metrics = REQUIRED_GUARDRAILS | {"Recall@10", "token_estimate", "cost_estimate"}
    missing_guardrail_metrics = required_guardrail_metrics - set(guardrail_metrics)
    if missing_guardrail_metrics:
        raise ValidationError(f"dataset.guardrail_metrics missing {sorted(missing_guardrail_metrics)}")
    ...
    if provenance.get("type") != "synthetic_and_public_seed":
        raise ValidationError("dataset provenance.type must be synthetic_and_public_seed")
    _reject_private_strings(provenance, "dataset.provenance")
    ...
    intents_by_split: dict[str, set[str]] = {"train": set(), "selection": set(), "test": set()}
    for index, query in enumerate(queries):
        if not isinstance(query, Mapping):
            raise ValidationError(f"queries[{index}] must be an object")
        _require_keys(query, REQUIRED_QUERY_FIELDS, f"queries[{index}]")
        query_id = _require_non_empty_string(query["query_id"], f"queries[{index}].query_id")
        _reject_private_text(query_id, f"queries[{index}].query_id")
        _reject_private_text(_require_non_empty_string(query["query_text"], f"queries[{index}].query_text"), f"queries[{index}].query_text")
        ...
        if split not in split_counts:
            raise ValidationError(f"queries[{index}].split must be train/selection/test")
        split_counts[split] += 1

        group_id = _require_non_empty_string(query["group_id"], f"queries[{index}].group_id")
        ...
        for enum_key, allowed_values in _ALLOWED_QUERY_ENUMS.items():
            value = _require_non_empty_string(query[enum_key], f"queries[{index}].{enum_key}")
            _reject_private_text(value, f"queries[{index}].{enum_key}")
            if value not in allowed_values:
                raise ValidationError(f"queries[{index}].{enum_key} must be one of {sorted(allowed_values)}")
        ...
        if not isinstance(must_include, list) or not must_include:
            raise ValidationError(f"queries[{index}].labels.must_include must be non-empty")
        if any(not isinstance(label, str) or not label.strip() for label in must_include):
            raise ValidationError(f"queries[{index}].labels.must_include must contain non-empty strings")
        for label_key, label_value in labels.items():
            if isinstance(label_value, str):
                _reject_private_text(label_value, f"queries[{index}].labels.{label_key}")
            elif isinstance(label_value, Sequence) and not isinstance(label_value, (str, bytes)):
                for value_index, value in enumerate(label_value):
                    if isinstance(value, str):
                        _reject_private_text(value, f"queries[{index}].labels.{label_key}[{value_index}]")

    if any(count == 0 for count in split_counts.values()):
        raise ValidationError(f"all splits must have at least one query: {split_counts}")
    if split_counts["selection"] < 2:
        raise ValidationError("selection split must include at least two queries for live SkillOpt gate stability")
...
def validate_execution_control(control: Mapping[str, Any]) -> None:
    ...
    _require_exact_policy(confidence_gate, _ALLOWED_CONFIDENCE_GATE, "query_analyzer_confidence_gate")
    ...
    _require_exact_policy(overlap_gate, _ALLOWED_OVERLAP_GATE, "improved_query_overlap_gate")
    ...
    _require_exact_policy(rollout_gate, _ALLOWED_CONTROLLED_ROLLOUT_GATE, "controlled_rollout_gate")
    ...
    _require_exact_policy(relevance, _ALLOWED_RELEVANCE_FILTER, "relevance_filter_policy")
    ...
    if any(not isinstance(item, str) or not item.strip() for item in metadata):
        raise ValidationError("required_rollout_metadata must contain non-empty strings")
...
def validate_candidate_artifact(
    ...
    *,
    ...
    _require_non_empty_string(artifact.get("version"), "candidate_artifact.version")
    skill_hash = _require_sha256_digest(artifact.get("skill_hash"), "candidate_artifact.skill_hash")
    baseline_hash = _require_sha256_digest(artifact.get("baseline_hash"), "candidate_artifact.baseline_hash")
    _require_non_empty_string(artifact.get("dataset_hash"), "candidate_artifact.dataset_hash")
    _require_non_empty_string(artifact.get("execution_control_hash"), "candidate_artifact.execution_control_hash")
    _require_iso_datetime(artifact.get("created_at"), "candidate_artifact.created_at")
    if skill_hash == baseline_hash:
        raise ValidationError("candidate_artifact.skill_hash must differ from baseline_hash")
    ...
    if snapshot.get("primary_metric") != PRIMARY_METRIC:
        raise ValidationError("candidate_artifact.metric_snapshot.primary_metric must be nDCG@10")
    baseline = snapshot.get("baseline")
    candidate = snapshot.get("candidate")
    baseline = _require_finite_number(baseline, "candidate_artifact.metric_snapshot.baseline")
    candidate = _require_finite_number(candidate, "candidate_artifact.metric_snapshot.candidate")
    ...
    if candidate < baseline:
        raise ValidationError("candidate_artifact candidate metric must not regress below baseline")

    guardrails = snapshot.get("guardrails")
    if not isinstance(guardrails, Mapping):
        raise ValidationError("candidate_artifact.metric_snapshot.guardrails is required")
    required_guardrails = set(dataset["guardrail_metrics"]) if dataset is not None else REQUIRED_GUARDRAILS
    missing_guardrails = required_guardrails - set(guardrails)
    if missing_guardrails:
        raise ValidationError(f"candidate_artifact guardrails missing {sorted(missing_guardrails)}")
    for name in required_guardrails:
        value = _require_finite_number(guardrails[name], f"candidate_artifact guardrail {name}")
        if name == "p95_latency_ms":
            if value < 0:
                raise ValidationError(f"candidate_artifact guardrail {name} must be non-negative")
            if value > _GUARDRAIL_LIMITS["p95_latency_ms"]:
                raise ValidationError(f"candidate_artifact guardrail {name} exceeds v1 threshold")
    ...
    if missing_rollout_metadata:
        raise ValidationError(f"candidate_artifact.rollout_metadata missing {sorted(missing_rollout_metadata)}")
    if rollout_metadata.get("skill_hash") != artifact.get("skill_hash"):
        raise ValidationError("candidate_artifact.rollout_metadata.skill_hash must match skill_hash")
    if rollout_metadata.get("dataset_hash") != artifact.get("dataset_hash"):
        raise ValidationError("candidate_artifact.rollout_metadata.dataset_hash must match dataset_hash")
    if rollout_metadata.get("execution_control_hash") != artifact.get("execution_control_hash"):
        raise ValidationError("candidate_artifact.rollout_metadata.execution_control_hash must match execution_control_hash")
    if rollout_metadata.get("hyde_enabled") is not False:
        raise ValidationError("candidate_artifact.rollout_metadata.hyde_enabled must be false")
    if rollout_metadata.get("cache_status") != "disabled_or_cold_per_rollout":
        raise ValidationError("candidate_artifact.rollout_metadata.cache_status must match execution control")
    if rollout_metadata.get("relevance_scorer_path") != "production_equivalent":
        raise ValidationError("candidate_artifact.rollout_metadata.relevance_scorer_path must be production_equivalent")
    if rollout_metadata.get("budget_branch") != "generous_fixed_offline_budget":
        raise ValidationError("candidate_artifact.rollout_metadata.budget_branch must match execution control")
    if not isinstance(rollout_metadata.get("query_analysis_confidence"), int | float) or isinstance(rollout_metadata.get("query_analysis_confidence"), bool):
        raise ValidationError("candidate_artifact.rollout_metadata.query_analysis_confidence must be numeric")
    query_analysis_confidence = float(rollout_metadata["query_analysis_confidence"])
    _require_metric_range(query_analysis_confidence, "candidate_artifact.rollout_metadata.query_analysis_confidence")
    if query_analysis_confidence < _MINIMUM_QUERY_ANALYSIS_CONFIDENCE:
        raise ValidationError("candidate_artifact.rollout_metadata.query_analysis_confidence below execution-control minimum")
    ...
    if rollback_to.get("skill_hash") != artifact.get("baseline_hash"):
        raise ValidationError("candidate_artifact.rollback_to.skill_hash must match baseline_hash")

    if dataset is not None and artifact.get("dataset_hash") != dataset.get("dataset_hash"):
        raise ValidationError("candidate_artifact.dataset_hash does not match dataset")
    if execution_control is not None and artifact.get("execution_control_hash") != execution_control.get("control_hash"):
        raise ValidationError("candidate_artifact.execution_control_hash does not match control")
...
def validate_rollback_record(
    ...
    *,
    ...
    _require_sha256_digest(record.get("from_candidate_hash"), "rollback_record.from_candidate_hash")
    _require_sha256_digest(record.get("rollback_to_skill_hash"), "rollback_record.rollback_to_skill_hash")
    reason = _require_non_empty_string(record.get("reason"), "rollback_record.reason")
    triggered_by = _require_non_empty_string(record.get("triggered_by"), "rollback_record.triggered_by")
    if reason not in {"guardrail_regression", "metric_regression", "scope_violation", "manual_rejection"}:
    ...
    if triggered_by not in {"offline_eval_gate", "shadow_eval_gate", "operator"}:
        raise ValidationError("rollback_record.triggered_by must be a known rollback trigger")
    if artifact is not None:
        if record.get("from_candidate_hash") != artifact.get("skill_hash"):
            raise ValidationError("rollback_record.from_candidate_hash does not match candidate")
        if record.get("rollback_to_skill_hash") != artifact.get("baseline_hash"):
            raise ValidationError("rollback_record.rollback_to_skill_hash does not match baseline")
...
def _require_exact_policy(policy: Mapping[str, Any], expected: Mapping[str, Any], label: str) -> None:
