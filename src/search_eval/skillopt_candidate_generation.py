"""Provenance wrapper for external SkillOpt candidate generation runs.
...
from .approved_policy import validate_runtime_policy_text
...
def record_candidate_generation_manifest(
    *,
    ...
    """Write a repo-native manifest for an externally generated SkillOpt candidate."""
    ...
    try:
        ...
        raise ValidationError(f"candidate best_skill path is unreadable: {skill_path}") from exc
    ...
    manifest = {
        ...
        "status": "candidate_generated_requires_offline_approval",
        ...
        "materialization_source_hashes": dict(materialization_manifest["source_hashes"]),
...
def validate_candidate_generation_manifest(manifest: Mapping[str, Any]) -> None:
    ...
    if missing:
        raise ValidationError(f"candidate generation manifest missing required keys: {sorted(missing)}")
    ...
    if manifest.get("status") != "candidate_generated_requires_offline_approval":
        raise ValidationError("candidate generation status must require offline approval")
    if manifest.get("requires_approved_export") is not True:
        raise ValidationError("candidate generation must require approved export")
    _require_iso_datetime(manifest.get("created_at"), "candidate_generation.created_at")
    materialization_path = _require_existing_file(manifest.get("materialization_manifest_path"), "materialization_manifest_path")
    if canonical_file_hash(materialization_path) != manifest.get("materialization_manifest_hash"):
        raise ValidationError("candidate generation materialization manifest hash mismatch")
    materialization_manifest = load_json(materialization_path)
    validate_skillopt_materialization_manifest(materialization_manifest)
    if dict(manifest.get("materialization_source_hashes", {})) != dict(materialization_manifest["source_hashes"]):
        raise ValidationError("candidate generation materialization source hashes mismatch")
    ...
    if manifest.get("execution_control_hash") != materialization_manifest.get("execution_control_hash"):
        raise ValidationError("candidate generation execution_control_hash mismatch")
    best_skill_path = _require_existing_file(manifest.get("best_skill_path"), "best_skill_path")
...
def _validate_external_run(external_run: Any) -> None:
    ...
    allowed = {"runner", "command", "run_id", "completed_at", "raw_user_logs_included", "pii_included"}
    if set(external_run) != allowed:
        raise ValidationError("candidate generation external_run contains unsupported keys")
    for key in ("runner", "command", "run_id"):
        value = external_run.get(key)
        if not isinstance(value, str) or not value.strip():
            raise ValidationError(f"candidate generation external_run.{key} is required")
        _reject_secret_like_text(value, f"candidate_generation.external_run.{key}")
    _require_iso_datetime(external_run.get("completed_at"), "candidate_generation.external_run.completed_at")
    if external_run.get("raw_user_logs_included") is not False:
        raise ValidationError("candidate generation external run must exclude raw user logs")
...
def _reject_secret_like_text(value: str, field: str) -> None:
    lowered = value.lower()
    for marker in ("api_key", "secret", "password", "raw user log", "raw_logs", "user_id", "session_id", "@"):
        if marker in lowered:
            raise ValidationError(f"{field} must not include secret, PII, or raw-user-log markers")
